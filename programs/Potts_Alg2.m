function [T1,C,iter] = Potts_Alg2(T1,C_prime,A,A1,q,X,tol,temp)
    ...
    while delta > tol
        ...
        delta = 0;
        for k = 1:X
            delta = delta + abs(sv(k,k)-sv_prime(k,k));
        end
        
        iter = iter + 1;
        if iter > maxiter
            disp(['Potts_Alg2 not converged at T = ' num2str(temp)]);
            break
        end
    end
end
