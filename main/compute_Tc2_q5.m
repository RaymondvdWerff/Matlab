...
%         disp(['Error from fit: ',num2str(abs(b-err_b))]);
        ...
        xshow = 0:max(xdata)/100:max(xdata);
...
%     xdata_show = ts(ts_beg-1):(ts(2)-ts(1))/100:ts(ts_end);
% 
%     myfittype = fittype('a*exp(b*(abs(c-x)/c)^(-1/2))');
%     myfittopts = fitoptions('Method','NonLinearLeastSquares','StartPoint',[5,1,0.96],'Weight',w);
...
% end
% errorbar(ts_begs,Tcs,errs,'.');hold on;
% xlabel('ending');
% ylabel('Tc');
